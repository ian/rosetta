...
/**
 * Legacy 0.x `translate-catalog` command, kept (deprecated) until v2.
...
/** Run the legacy command from raw argv (starting with `translate-catalog`). */
export async function runLegacy(argv: string[]): Promise<void> {
	...
	await runCatalog(parsed, await buildClient(parsed));
}
